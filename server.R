  ...
  observe({
    ...
    updatePickerInput(session, "allele", choices =  alleles_sorted$Allele,  choicesOpt = list(subtext = paste0(alleles_sorted$functional_status, " (n = ", alleles_sorted$n, ")")))
      ...
      alleles_sorted <-  rv$raw_data_for_allele_picker %>% 
        separate_rows(Studies, sep = ", ") %>% distinct(Allele, Studies) %>% count(Allele) 
  ...
  observeEvent(debounced_click_dt(), {
    ...
    tmp <-   switch(input$mapping_method, 
        ...
        if(input$mapping_method == "Countries" & gr_country == "USA"){
          tmp <-   tmp %>% mutate(subgroup=subsubgroup) %>% 
    ...
    rv$raw_data_for_download %>% 
        ...
        DT::datatable(.,escape = F, rownames = F,
                  selection = "none",
                  options = list(dom="lpti", paging = T, scrollX=TRUE, scrollCollapse=F,
    ...
    shinyalert::shinyalert(
      ...
      text =if(!(input$allele == "*1" | grepl("Ref|ref", input$allele))){
        ...
        tagList(DT::DTOutput("raw_data_sweet_dt"),
                br(),
...
debounced_click <- debounce(reactive(input$leaflet_base_shape_click), 500)
  ...
  observeEvent( debounced_click(), { 
    ...
    shinyalert::shinyalert(
      ...
      text = if(!(input$allele == "*1" | grepl("Ref|ref", input$allele))){
        ...
        tagList(DT::DTOutput("raw_data_sweet2"),
                br(),
  ...
  # # leaflet  ----------------------------------------------------------------
  # observeEvent(input$type_of_plot == "leaflet", {
...
output$dl_leaflet <- downloadHandler(
    filename = function() {paste0("pharmfreq_", input$gene,gsub("[*]","_",input$allele),".png")},
  ...
  output$rrender_add_info <- renderUI({
    ...
    if(unique(rv$haplotype_allele$comment) == 'pharmvar'){
      ...
      b_hap <- argonCard(
        ...
        br(),
        div(glue("{ifelse(n_distinct(rv$haplotype_allele$rs_id)==1, paste(tmp1), paste(gsub('XX', n_distinct(rv$haplotype_allele$rs_id),tmp2)))} {paste(tmp3)}"),
    ...
    if(gene_lgl){
      ...
      if (hap_lgl & status_lgl & gene_lgl)     {list(br(), fluidRow(column(width = 5,b_status,
                                                                                     gd_status),
                                                                    column(width = 7,b_hap)))}
    else if (!hap_lgl & status_lgl & gene_lgl) {list(br(), fluidRow(column(width = 6,b_status,
                                                                           gd_status)  ))}
    else if (hap_lgl & !status_lgl & gene_lgl) {list(br(), fluidRow(
      column(width = 6,gd_status),
      column(width = 6,b_hap)))}
    else if (hap_lgl & !status_lgl & !gene_lgl) {list(br(), fluidRow( column(width = 6,b_hap)))}
    else if (!hap_lgl & status_lgl & !gene_lgl) {list(br(), fluidRow( column(width = 6,b_status)))}
    else if (!hap_lgl & !status_lgl & gene_lgl) {list(br(), fluidRow( column(width = 6,gd_status)))}
...
# tool 1 tool1------------------------------------------------------------------
 ...
 output$tool_act_plot <- renderPlotly({
   ...
   pp <- rv$tool_act_plot_data %>% 
     ...
     scale_y_continuous(filter(CMS_text, what=="Tool1")$text, labels = scales::percent)+
...
# tool 2 tool2------------------------------------------------------------------
 ...
 output$tool2_downloadData <- downloadHandler(
   ...
   content = function(file) {
     write.table(select(rv$tools_freq_data,
                        subgroup, country, frequency=	Weighted) , file, quote = F, row.names = F, sep="\t")
   }
 )
 ...
 if(is.null(rv$df_segment)){
     ...
     scale_x_continuous(filter(CMS_text, what=="Tool3")$text, labels = scales::percent, limits = c(0, NA))+
     scale_y_discrete("")+
     theme_bw(base_size = 16) +
     theme(legend.position = "bottom")
 }else{
 ...
 output$tool3_downloadData <- downloadHandler(
   ...
   content = function(file) {
     write.table( select(rv$freq_variability_server, 
                         subgroup, country,	gene,	allele=Allele, frequency=Weighted,	maximal_difference=max_dif) , file, quote = F, row.names = F, sep="\t")
   }
 )
